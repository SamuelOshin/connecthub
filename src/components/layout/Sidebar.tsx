...
import { Compass, Heart, MessageCircle, User, Settings, Zap } from "lucide-react";
...
export function Sidebar({ className }: { className?: string }) {
  ...
  return (
    <nav className={cn("w-64 flex flex-col h-full bg-white dark:bg-[#1a242f] border-r border-slate-200 dark:border-slate-800 z-20 shadow-sm", className)}>
      <div className="p-4 pt-6 flex flex-col gap-2 flex-1">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              />
              <span className={cn("text-sm font-bold", !isActive && "font-medium")}>
                {item.label}
              </span>
              {item.badge && (
                <span className={cn(
      ...
      <div className="p-4 mt-auto space-y-4">
        ...
        <div className="border-t border-slate-100 dark:border-slate-800 pt-4">
          <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer transition-colors">
            <div 
